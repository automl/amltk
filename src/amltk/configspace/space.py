...
class ConfigSpaceAdapter(SpaceAdapter[InputSpace, ConfigurationSpace]):
    ...
    [`Parser`][amltk.pipeline.Parser] interface and sampling from them with
    ...
    def parse_space(
        ...
        if space is None:
            ...
            _space = ConfigurationSpace(space)
        elif isinstance(space, Hyperparameter):
            _space = ConfigurationSpace({space.name: space})
        elif isinstance(space, ConfigurationSpace):
            _space = self.copy(space)
        else:
            raise TypeError(f"{space} is not parsable as a space")
    ...
    def _sample(
        ...
        """See [`Sampler._sample`][amltk.pipeline.Sampler._sample]."""
        if seed:
