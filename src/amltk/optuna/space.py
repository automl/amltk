...
The notable class is [`OptunaSpaceAdapter`][amltk.optuna.OptunaSpaceAdapter].
It implements the [`SpaceAdapter`][amltk.pipeline.SpaceAdapter] interface
...
4. Pass the `adapter` to [`space()`][amltk.pipeline.Step.space] to get the optuna space.
    It will be a dictionary mapping the name of the hyperparameter to the optuna
    distribution.
5. Pass the `adapter` to [`sample()`][amltk.pipeline.Step.sample] to get a sample
    config. It will be a dictionary mapping the name of the hyperparameter to the
    sampled value.
6. Create an instance of [`OptunaSpaceAdapter`][amltk.optuna.OptunaSpaceAdapter] which
    will be used to parse the space and sample from it.

    !!! note "Note"

        The `OptunaSpaceAdapter` is a [`SpaceAdapter`][amltk.pipeline.SpaceAdapter]
        which means that it can be used to parse any space and sample from it. It
        implements the [`Parser`][amltk.pipeline.Parser] and
        [`Sampler`][amltk.pipeline.Sampler] interfaces.
"""
...
class OptunaSpaceAdapter(SpaceAdapter[OptunaSearchSpace]):
    ...
    def insert(
        ...
        *,
        ...
        """See [`Parser.insert`][amltk.pipeline.Parser.insert]."""
    ...
    def condition(
        ...
        """See [`Parser.condition`][amltk.pipeline.Parser.condition]."""
