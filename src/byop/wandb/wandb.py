...
from byop.scheduling import Scheduler, SequentialExecutor, Task, TaskPlugin
...
WRun: TypeAlias = Union[wandb_run.Run, RunDisabled]
...
class WandbParams:
    ...
    [`WandbPlugin`][byop.wandb.WandbPlugin] to initialize a run. It can be
    modified using the [`modify()`][byop.wandb.WandbParams.modify] method.
...
class WandbLiveRunWrap:
    ...
    [`WandbTrialTracker`][byop.wandb.WandbTrialTracker] to wrap
    the target function.
    """
...
class WandbPlugin:
    ...
    can be used to create a [`trial_tracker()`][byop.wandb.WandbPlugin.trial_tracker]
    to pass into a [`Trial.Task(plugins=...)`][byop.optimization.Trial.Task] or to
    create `wandb.Run`'s for custom purposes with [`run()`][byop.wandb.WandbPlugin.run].
    """
