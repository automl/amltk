...
class TaskPlugin(ABC, Generic[P, R]):
    ...
    [`Task`][byop.scheduling.Task]. A plugin can affect, modify and extend its
    ...
    that a [`Task`][byop.scheduling.Task] emits.

    For an example of a simple plugin, see the
    [`CallLimiter`][byop.scheduling.CallLimiter] plugin which prevents
...
class CallLimiter(TaskPlugin[P, R]):
    ...
    from byop.scheduling import CallLimiter, Task, Scheduler
