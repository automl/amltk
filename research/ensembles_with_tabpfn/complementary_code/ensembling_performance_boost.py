...
def _get_ensemble_method_predictions(base_models: List[BaseModel], n_iterations, loss_function_proba_in, n_jobs, seed,
    ...
    # TODO: base on iterations, time or evals? decide for one
    ...
    val_loss = ens.validation_loss_

    for bm in base_models:
        bm.switch_to_test_simulation()

    return ens.predict_proba(X_test), val_loss


def get_data_for_performance_increase_with_new_model(base_models: List[BaseModel], complement_model: BaseModel,
                                                     loss_function_proba_in: Callable,
                                                     score_function_proba_in: Callable,
                                                     loss_to_score_function: Callable, seed: int,
    ...
    loss_to_score_function:
        A function that takes a loss value and returns its score representative.
    seed: int
    ...
    bm_predictions, bm_val_loss = _get_ensemble_method_predictions(base_models, n_iterations, loss_function_proba_in,
                                                                   n_jobs, seed,
                                                                   X_val, y_val, X_test)
    bm_with_complement_predictions, bm_c_val_loss = _get_ensemble_method_predictions(base_models + [complement_model],
                                                                                     n_iterations,
                                                                                     loss_function_proba_in, n_jobs,
                                                                                     seed,
                                                                                     X_val, y_val, X_test)
    ...
    val_score_bm = loss_to_score_function(bm_val_loss)
    val_score_bm_c =  loss_to_score_function(bm_c_val_loss)

    return bm_predictions, bm_with_complement_predictions, score_bm, score_bm_with_complement, val_score_bm, val_score_bm_c
